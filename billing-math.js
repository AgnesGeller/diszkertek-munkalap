(function(root){
  ...
  function build(item,prices){
    ...
    for(let i=1;i<=3;i++){
      ...
      result.push({label:valid?`Csapat ${i}: ${size} fő, ${from}–${to} (${b-a} perc/fő)`:`Csapat ${i}: hiányos vagy hibás munkaidő, ellenőrizendő`,quantity:String(valid?n*(b-a):0),unit:'főperc',unitPrice:String(labor.unit_price),divisor:60,reviewed:valid&&labor.confirmed});
    }
